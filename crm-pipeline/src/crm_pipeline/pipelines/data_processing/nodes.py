...
def _create_input_table(
    ...
    crm_interactions = crm.merge(interactions, left_on="customer_id", right_on="customers")
    raw_output_table = crm_interactions.merge(products, left_on="month_of_year", right_on="date")
    raw_output_table.rename(columns={'product':'product_of_interaction'},inplace=True)
    df=raw_output_table[["customer_id","occupation","type","product_of_interaction","event"]]
    df['freq']=1
    return df
...
def create_pivot_table(
    ...
    df_pivot=model_input_table.pivot_table(index=["customer_id","occupation","type","product_of_interaction"],columns=["event"],values='freq',aggfunc='count')
    return df_pivot